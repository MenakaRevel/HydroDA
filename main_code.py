...
def main_act(exp,ifact):
    ...
    global exp_dir, ifactor

    #exp_dir=pm.SWOT_dir()+"/out/"+pm.experiment()
    exp_dir=pm.SWOT_dir()+"/out/"+exp
    ifactor=float(ifact)
    ...
    shutil.copy("params.py",exp_dir+"/assim_out/")
    ...
    for day in np.arange(days_count):
        ...
        one_day_loop(yyyy,mm,dd,day,exp_dir)
...
## single loop program
################
def one_day_loop(yyyy,mm,dd,day,exp_dir):
    print "================================ start loop of "+yyyy+" "+mm+" "+dd+" ========================================"
    #
#    # True Simulation ####################################################################
    if pm.run_flag() == 0 or pm.run_flag() == 2:
        one_day_sim([yyyy,mm,dd,"000","true",exp_dir])
    ...
    if pm.run_flag() == 0 or pm.run_flag() == 1:
        ...
        for ens_num in np.arange(1,pm.ens_mem()+1):
            ODM_inputlist.append([yyyy,mm,dd,'%03d'%ens_num,"open",exp_dir])
    ...
    # set for ensemble simulations
    for ens_num in np.arange(1,pm.ens_mem()+1):
        ODM_inputlist.append([yyyy,mm,dd,'%03d'%ens_num,"assim",exp_dir])
    ...
    data_assim(yyyy,mm,dd,day,exp_dir)
...
#            return 0
    ...
    if pm.spinup_mode()==0 or pm.spinup_mode()==2:
        inputlist.append([yyyy,"true",'000',exp_dir])
        #spinup_loop(inputlist)

    if pm.spinup_mode()==0 or pm.spinup_mode()==1:
        for ens_num in np.arange(1,pm.ens_mem()+1):
             inputlist.append([yyyy,"open",'%03d'%ens_num,exp_dir])
...
def spinup_loop(inputlist):
    ...
    os.system("source src/spin_up.sh "+str(yyyy)+" "+str(loop)+" "+ens_num+" "+dir2+" "+str(cpunums)+" "+str(run_name)+" "+exp_dir)
    return 0
###########################
def one_day_sim(inputlist):
    ...
    cpunums = pm.cpu_nums()
    os.system("source src/oneday_sim.sh "+yyyy+" "+mm+" "+dd+" "+ens_num+" "+dir2+" "+looptype+" "+str(cpunums)+" "+str(run_name)+" "+exp_dir)

    if looptype=="true":
        # copying "restart file" to ./CaMa_in/
        thisday=datetime.date(int(yyyy),int(mm),int(dd))
        nxt_day=thisday+datetime.timedelta(days=1)
        orgrestf=exp_dir+"/CaMa_out/"+yyyy+mm+dd+"T"+ens_num+"/restart"+'%04d'%(nxt_day.year)+'%02d'%(nxt_day.month)+'%02d'%(nxt_day.day)+".bin"
        newrestf=exp_dir+"/CaMa_in/restart/"+looptype+"/restart"+'%04d'%(nxt_day.year)+'%02d'%(nxt_day.month)+'%02d'%(nxt_day.day)+"T000.bin"
        #os.system("cp "+orgrestf+" "+newrestf)
        copy_stoonly(orgrestf,newrestf)

        # copying "WSE" as "xa_m" in ./assim_out
        oldfname=exp_dir+"/CaMa_out/"+yyyy+mm+dd+"T"+ens_num+"/sfcelv"+yyyy+".bin"
        newfname=exp_dir+"/assim_out/xa_m/"+looptype+"/"+yyyy+mm+dd+"_xam.bin"
...
def copy_corrupted_sfcelv(inputlist):
    ...
    num  = inputlist[3]
    numch='%03d'%num
    fname=exp_dir+"/CaMa_out/"+yyyy+mm+dd+"C"+numch+"/sfcelv"+yyyy+".bin"
    os.system("cp "+fname+" "+exp_dir+"/assim_out/ens_xa/open/"+yyyy+mm+dd+"_"+numch+"_xa.bin")
    return 0
...
def copy_corrupted_restart(inputlist):
    ...
    numch='%03d'%num
    fname=exp_dir+"/CaMa_out/"+yyyy+mm+dd+"C"+numch+"/restart"+n_yyyy+n_mm+n_dd+".bin"
    #os.system("cp "+fname+" ./CaMa_in/restart/open/restart"+n_yyyy+n_mm+n_dd+"C"+numch+".bin")
    copy_stoonly(fname,exp_dir+"/CaMa_in/restart/open/restart"+n_yyyy+n_mm+n_dd+"C"+numch+".bin")
...
def assim_at_fort(yyyy,mm,dd,day): #previous --> used
    ...
    return 0
###########################
def data_assim(yyyy,mm,dd,day,exp_dir): # new data assimilation function (2017-06-30)
    errrand=np.random.normal(0,pm.ovs_err())
    err_rand(exp_dir) # make white noise for observation
    dir1=pm.CaMa_dir()+"/"
    thisday=datetime.date(int(yyyy),int(mm),int(dd))
    nxt_day=thisday+datetime.timedelta(days=1)
...
#    os.system("src/data_assim "+str(pm.assimN())+" "+str(pm.assimS())+" "+str(pm.assimW())+" "+str(pm.assimE())+" "+yyyy+mm+dd+" "+str('%02d'%SWOT_day(yyyy,mm,dd))+" "+str(pm.patch_size())+" "+str(pm.ens_mem())+" "+str(day)+" "+str('%04d'%(nxt_day.year)+'%02d'%(nxt_day.month)+'%02d'%(nxt_day.day))+" "+str(pm.err_expansion())+" "+dir1+" "+str(errrand)+" "+str(pm.ovs_err())+" "+str(pm.thersold()))
    os.system("src/data_assim "+str(pm.assimN())+" "+str(pm.assimS())+" "+str(pm.assimW())+" "+str(pm.assimE())+" "+yyyy+mm+dd+" "+str('%02d'%SWOT_day(yyyy,mm,dd))+" "+str(pm.patch_size())+" "+str(pm.ens_mem())+" "+str(day)+" "+str('%04d'%(nxt_day.year)+'%02d'%(nxt_day.month)+'%02d'%(nxt_day.day))+" "+str(pm.err_expansion())+" "+dir1+" "+str(errrand)+" "+str(pm.ovs_err())+" "+str(pm.thersold())+" "+exp_dir+" "+pm.SWOT_dir()+" "+pm.patch_dir()+" "+pm.patch_id()+" "+str(ifactor))
...
def make_rand(std): #used
    ...
    randlist.tofile(exp_dir+"/CaMa_out/randlist.bin")
###########################
def initial(exp_dir): #used
    # program for initialization

    # creating output folders
    mkdir(pm.SWOT_dir()+"/out")

    mkdir(exp_dir)
    mkdir(exp_dir+"/CaMa_out")
    mkdir(exp_dir+"/CaMa_in")
    mkdir(exp_dir+"/CaMa_in/restart")
    mkdir(exp_dir+"/CaMa_in/restart/assim")
    mkdir(exp_dir+"/CaMa_in/restart/open")
    mkdir(exp_dir+"/CaMa_in/restart/true")
    mkdir(exp_dir+"/assim_out")
    mkdir(exp_dir+"/assim_out/xa_m")
    mkdir(exp_dir+"/assim_out/xa_m/assim")
    mkdir(exp_dir+"/assim_out/xa_m/open")
    mkdir(exp_dir+"/assim_out/xa_m/true")

    mkdir(exp_dir+"/assim_out/ens_xa")         # NEW v.1.1.0
    mkdir(exp_dir+"/assim_out/ens_xa/assim")   # NEW v.1.1.0
    mkdir(exp_dir+"/assim_out/ens_xa/open")    # NEW v.1.1.0

    mkdir(exp_dir+"/assim_out/nonassim")
    mkdir(exp_dir+"/assim_out/nonassim/open")
    mkdir(exp_dir+"/assim_out/nonassim/assim")

    mkdir(exp_dir+"/assim_out/rest_true")
    mkdir(exp_dir+"/assim_out/rivout")
    mkdir(exp_dir+"/assim_out/rivout/open")
    mkdir(exp_dir+"/assim_out/rivout/assim")
    mkdir(exp_dir+"/assim_out/rivout/true")
    mkdir(exp_dir+"/assim_out/fldout")
    mkdir(exp_dir+"/assim_out/fldout/open")
    mkdir(exp_dir+"/assim_out/fldout/assim")
    mkdir(exp_dir+"/assim_out/fldout/true")
    mkdir(exp_dir+"/assim_out/flddph")
    mkdir(exp_dir+"/assim_out/flddph/open")
    mkdir(exp_dir+"/assim_out/flddph/assim")
    mkdir(exp_dir+"/assim_out/flddph/true")
    mkdir(exp_dir+"/err_out")
    mkdir(exp_dir+"/assim_out/fldarea/")
    mkdir(exp_dir+"/assim_out/fldarea/open")
    mkdir(exp_dir+"/assim_out/fldarea/assim")
    mkdir(exp_dir+"/assim_out/fldarea/true")
    # inflation parameter
    mkdir(exp_dir+"/inflation")

    os.system("touch "+exp_dir+"/assim_out/__init__.py")
    mkdir(exp_dir+"/logout")
    return 0
###########################
...
def make_restart(inputlist):
    ...
    dir1=pm.CaMa_dir()+"/"
    print "dir1",dir1
    os.system("./src/make_restart "+yyyy+mm+dd+" "+yyyy_b+mm_b+dd_b+" "+yyyy_n+mm_n+dd_n+" "+loop+" "+dir1+" "+str(pm.ens_mem())+" "+numch+" "+exp_dir)
...
def courrpt_rivhgt():
    ...
    return 0
###########################
def make_initial_infl(exp_dir):
    parm_infl=np.ones([720,1440],np.float32)*pm.initial_infl()
    start_year,start_month,start_date=pm.starttime() # Start year month date
    yyyy='%04d' % (start_year)
    mm='%02d' % (start_month)
    dd='%02d' % (start_date)
    parm_infl.tofile(exp_dir+"/inflation/parm_infl"+yyyy+mm+dd+".bin")
###########################
def mkdir(path):
    try:
        os.makedirs(path)
...
def cal_monthly_mean_ens(ens_num): #start_year,end_year,ens_num,months=24):
    ...
    mkdir(exp_dir+"/CaMa_in/"+runname+"/mean_month")
    ...
    for month in np.arange(months):
        ...
        for day in np.arange(month*30,(month+1)*30):
            ...
            mm='%02d' % (running_dt.month)
            dd='%02d' % (running_dt.day)
            roff=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+str(yyyy)+str(mm)+str(dd)+ens_char+".one",np.float32).reshape([ny,nx])
        ...
        roff_mean.tofile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+ychar+mchar+ens_char+".bin")
###########################
def cal_monthly_total(start_year,end_year,months=24,threshold=0.1):
    ...
    if runname=="ERA20CM":
        ...
        prefix="Roff__"
        suffix="%03d.one"%(true_run)
    mkdir(exp_dir+"/CaMa_in/"+runname+"/total_month")
    #os.system("rm -Rf ./CaMa_in/ELSE_GPCC/mean_month/*")
    ...
    for month in np.arange(months):
        ...
        for day in np.arange(month*30,(month+1)*30):
            ...
            mm='%02d' % (running_dt.month)
            dd='%02d' % (running_dt.day)
#            roff=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/"+prefix+str(yyyy)+str(mm)+str(dd)+suffix,np.float32).reshape([ny,nx])
            roff=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/"+"Roff__"+str(yyyy)+str(mm)+str(dd)+"T000.one",np.float32).reshape([ny,nx])
            roff_mon=roff_mon+roff*(roff>threshold)
            #count=count+(roff>threshld)
        roff_total=roff_mon #/(count+1e-20)
        roff_total=roff_total.astype(np.float32)
        roff_total=roff_total+threshold
        roff_total.tofile(exp_dir+"/CaMa_in/"+runname+"/total_month/total_"+ychar+mchar+".bin")
###########################
def cal_monthly_mean(start_year,end_year,months=24):
    ...
    for month in np.arange(months):
        ...
        roff_mean=roff_mean.astype(np.float32)
        roff_mean=roff_mean+threshold
        roff_mean.tofile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+ychar+mchar+".bin")
###########################
...
def make_rivman():
    if pm.rivman_error()==1:
        ...
        # rivnum
        rivnum=pm.SOWT_dir+"/data/rivnum.bin"
        rivnum=np.fromfile(rivnum,np.int32).reshape(720,1440)

        # rivman
        rivman=np.ones([720,1440],np.float32)*base_man
        #print np.amax(rivnum)
        for riv in np.arange(1,1000): #np.amax(rivnum)+1):
             #print riv
             num=float(riv)
             ...
             for i in np.arange(len(index[0])):
                 ...
                 iy=index[0][i]
                 w=rivwth[iy,ix]
                 rivman[iy,ix]=nmin+(nmax-nmin)*((wmax-w)/((wmax-wmin)+1.0e-20))
...
def observation_error(exp_dir):
    ...
    obs_err=ma.masked_where(area<1.0,obs_err).filled(0.25) # 25cm for < 1km^2 water area
    obs_err=obs_err*nextx
    obs_err=obs_err.astype(np.float32)
    obs_err.tofile(exp_dir+"/data/obs_err.bin")
    return 0
#####################################################################
def err_rand(exp_dir):
    ...
    rand.tofile(exp_dir+"/CaMa_out/errrand.bin")
    return 0
###########################
def prepare_input(exp_dir):
    # spinup start_year
    # simulation end_year
    start_year=pm.spinup_end_year()
    end_year,end_month,end_date=pm.starttime()
    ...
    if pm.mode()==1: # Earth2Observe
        ...
        if(len(glob.glob(exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff*"))!=0):
            print "TRUE available"
            pass
        # true input file is not ready
        # need preparation
        # make directories
        mkdir(exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE")
        #print "E2O/Roff_TRUE"
        inputlist=[]
        print "prepare true"
        for day in np.arange(start,last):
            ...
            iname=pm.SWOT_dir()+"/inp/"+runname+"/Roff/Roff__"+yyyy+mm+dd+true_char+".one" #as true
            oname=exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff__"+yyyy+mm+dd+ens_char+".one"
        ...
        if(len(glob.glob(exp_dir+"/CaMa_in/"+runname+"/Roff_CORR/Roff*"))!=0):
            print "Roff_CORR available"
            pass
        # corrupted input file is not ready
        # need preparation
        # make directories
        mkdir(exp_dir+"/CaMa_in/"+runname+"/Roff_CORR")
        #print "E2O/Roff"
        ...
        for day in np.arange(start,last):
            ...
            for runens in open_list: #np.arange(1,7+1):
                ...
                #if runens!=true_run:
                run_num="%03d"%(runens)
                iname=exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+yyyy+mm+dd+run_num+".one"
                ifile=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+yyyy+mm+dd+run_num+".one",np.float32).reshape(720,1440)
                roff_mean=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+yyyy+mm+run_num+".bin",np.float32).reshape(720,1440)
                ...
                for dist in distopens:
                    ...
                    oname=exp_dir+"/CaMa_in/"+runname+"/Roff_CORR/Roff__"+yyyy+mm+dd+ens_char+".one"
    ...
    if pm.mode()==2: #ECMWF ERA20CM
        ...
        for day in np.arange(start,last):
            ...
            ens_char="T000"
            true_char="%03d"%(true_run)
            iname=exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+yyyy+mm+dd+true_char+".one"
            oname=exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff__"+yyyy+mm+dd+ens_char+".one"
            inputlist.append([iname,oname,"1.00"])
        ...
        for day in np.arange(start,last):
            ...
            for runens in open_list: #np.arange(1,10+1):
                run_num="%03d"%(runens)
                iname=exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+yyyy+mm+dd+run_num+".one"
                ifile=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff/Roff__"+yyyy+mm+dd+run_num+".one",np.float32).reshape(180,360)
                roff_mean=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+yyyy+mm+run_num+".bin",np.float32).reshape(180,360)
    ...
    if pm.mode()==3: # ELSE Kim 2009/E2O/ERA20CM
        ...
        for day in np.arange(start,last):
            ...
            if runname=="ERA20CM":
                ...
                prefix="Roff__"
                suffix="%03d.one"%(true_run)
            iname=pm.SWOT_dir()+"/inp/"+runname+"/Roff/"+prefix+yyyy+mm+dd+suffix
            #print iname
            oname=exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff__"+yyyy+mm+dd+ens_char+".one"
            inputlist.append([iname,oname,"1.00"])
        ...
        for day in np.arange(start,last):
            ...
            #std=std.astype(np.float32)
            #print std
            ifile=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff__"+yyyy+mm+dd+"T000.one",np.float32).reshape(ny,nx)
            roff_mean=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+yyyy+mm+".bin",np.float32).reshape(ny,nx)
            roff_total=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/total_month/total_"+yyyy+mm+".bin",np.float32).reshape(ny,nx)
            for ens in np.arange(1,pm.ens_mem()+1):
                ens_char="C%03d"%(ens)
                ...
                ofile.tofile(exp_dir+"/CaMa_in/"+runname+"/Roff_CORR/Roff__"+yyyy+mm+dd+ens_char+".one")
    ...
    if pm.mode()==4: # ELSE_KIM2009 , differnt yeaer
        ...
        for day in np.arange(start,last):
            ...
            dd='%02d' % (target_dt.day)
            ens_char="T000"
            iname=exp_dir+"/CaMa_in/"+runname+"/Roff/Roff____"+yyyy+mm+dd+".one"
            oname=exp_dir+"/CaMa_in/"+runname+"/Roff_TRUE/Roff__"+yyyy+mm+dd+ens_char+".one"
            inputlist.append([iname,oname,"1.00"])
        ...
        for day in np.arange(start,last):
            ...
            std=std.astype(np.float32)
            #print std
            ifile=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/Roff/Roff____"+yyyy+mm+dd+".one",np.float32).reshape(ny,nx)
            roff_mean=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/mean_month/mean_"+yyyy+mm+".bin",np.float32).reshape(ny,nx)
            roff_total=np.fromfile(exp_dir+"/CaMa_in/"+runname+"/total_month/total_"+yyyy+mm+".bin",np.float32).reshape(ny,nx)
            for ens in np.arange(1,pm.ens_mem()+1):
                ens_char="C%03d"%(ens)
